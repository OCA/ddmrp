...
class StockBuffer(models.Model):
    ...
    def _prepare_history_data(self):
        data = super(StockBuffer, self)._prepare_history_data()
        if self.simulation_id:
            simulation_line = self.simulation_id.simulation_line_ids.search([
                ('simulation_id', '=', self.simulation_id.id),
                ('product_id', '=', self.product_id.id),
                ('date', '=', fields.Date.today()),
            ], limit=1)
            if simulation_line:
                data['on_hand_simulation'] = simulation_line.on_hand
        return data

    # TODO: Refactory ddmrp_history to support new features
    def _compute_history_chart(self):
        ...
        for rec in self:
            ...
            p.line(dates, data["on_hand_simulation"], line_width=3, line_dash="dashed")
    ...
    def _compute_execution_history_chart(self):
        ...
        for rec in self:
            ...
            p.line(dates, data["on_hand_position"], line_width=3, line_dash="dotted")
            p.line(dates, data["on_hand_simulation"], line_width=4, line_dash="dashed")

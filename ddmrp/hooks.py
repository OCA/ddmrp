...
def post_load_hook():
    ...
    def new_get_pdf_line(self, bom_id, product_id=False, qty=1,
                         child_bom_ids=None, unfolded=False):

        if not hasattr(self, "_get_line_vals"):
            return self._get_pdf_line(self, bom_id, product_id=False, qty=1,
                                      child_bom_ids=None, unfolded=False)
        ...
        def new_get_sub_lines(bom, product_id, line_qty, line_id, level):
            data = self._get_bom(
                bom_id=bom.id, product_id=product_id.id, line_qty=line_qty,
            ...
            for bom_line in bom_lines:
                ...
                if bom_line["child_bom"] and (
                    ...
                    lines += (new_get_sub_lines(
                        ...
                        bom_line["prod_qty"], line, level + 1))
            if data["operations"]:
                lines.append({
                    "name": _("Operations"),
                    "type": "operation",
    ...
    if not hasattr(ReportBomStructure, "_get_pdf_line_original"):
        ReportBomStructure._get_pdf_line_original = \
            ReportBomStructure._get_pdf_line

odoo.define('ddmrp.list_renderer_buffer_info', function (require) {
    ...
    ListRenderer.include({
        ...
        _renderBodyCell: function (record, node, colIndex, options) {
            ...
            var options = node.attrs.options
            ...
            if (options) {
                if (options['color_from']) {
                    ...
                    $td.children().on('click', {record: record}, this._onCellClickListener.bind(this));
        ...
        _onBodyClickListener: function (event) {
        ...
        _onCellClickListener: function (event) {
            ...
            var genericCloseBtnHtml = '<button onclick="$(this).closest(\'div.popover\').popover(\'hide\');" type="button" class="close">&times;</button>';

            // Data request
            this.detailDP.add(this._rpc({
                model: 'stock.buffer',
        ...
        _onMouseLeaveListener: function (event) {
            var target = event.currentTarget;
            this._dismissAllPopovers();
        },
    });

...
class DdmrpDuplicateBuffer(models.TransientModel):
    ...
    product_id = fields.Many2one(comodel_name="product.product", string="New Product",)
    location_id = fields.Many2one(comodel_name="stock.location", string="New Location",)
    ...
    def action_duplicate_buffer(self):
        ...
        for buffer in buffer_obj.browse(buffer_ids):
            ...
            if self.type in ["location", "both"]:
                ...
                default["warehouse_id"] = self.location_id.get_warehouse().id

...
class StockBuffer(models.Model):
    ...
    lead_days = fields.Integer(
        "Lead Time (Distributed)", default=1,
    ...
    def _check_product_uom(self):
        if any(buffer.product_id.uom_id.category_id !=
               buffer.product_uom.category_id for buffer in self):
    ...
    def _check_procure_uom(self):
        if any(buffer.product_uom and
               buffer.procure_uom_id and
               buffer.product_uom.category_id !=
               buffer.procure_uom_id.category_id
               for buffer in self):
            raise ValidationError(_(
    ...
    def _compute_product_available_qty(self):
        ...
        for location_id, buffer_in_location in operation_by_location.items():
            ...
            products_not_res = buffer_in_location.mapped(
            ...
            for buffer in buffer_in_location:
                ...
                buffer.update({
                    ...
                    "product_location_qty_available_not_res":
                        product_not_res["qty_available_not_res"],
                })
    ...
    @api.depends(
        "dlt", "adu", "buffer_profile_id.lead_time_id.factor", "red_override",
        "buffer_profile_id.variability_id.factor", "product_uom.rounding",
    ...
    def _adjust_procure_qty(self, qty):
        ...
        remainder = self.qty_multiple > 0 and \
            adjusted_qty % self.qty_multiple or 0.0
        if float_compare(remainder, 0.0, precision_rounding=rounding) > 0:
            adjusted_qty += self.qty_multiple - remainder
        if float_compare(adjusted_qty, self.procure_min_qty,
    ...
    def get_ddmrp_chart(self):
        ...
        p.toolbar.logo = None
        red = p.vbar(x=1, bottom=0, top=self.top_of_red, width=1,
                     color="red", legend=False)
        yellow = p.vbar(x=1, bottom=self.top_of_red, top=self.top_of_yellow,
                        width=1, color="yellow", legend=False)
        green = p.vbar(x=1, bottom=self.top_of_yellow, top=self.top_of_green,
                       width=1, color="green", legend=False)
        net_flow = p.line(
            [0, 2], [self.net_flow_position, self.net_flow_position],
            line_width=2)
        on_hand = p.line(
            [0, 2], [self.product_location_qty, self.product_location_qty],
        ...
        labels_source_data = {
            "height": [self.net_flow_position,
                       self.product_location_qty,
                       self.top_of_red,
                       self.top_of_yellow,
                       self.top_of_green],
            "weight": [0.25, 1.75, 1, 1, 1],
            "names": [self.net_flow_position,
                      self.product_location_qty,
                      self.top_of_red,
                      self.top_of_yellow,
                      self.top_of_green],
    ...
    def _calc_adu_past_demand(self):
        ...
        date_from = fields.Date.to_string(self.warehouse_id.wh_plan_days(
            datetime.now(), -1 * horizon))
        date_to = fields.Date.to_string(self.warehouse_id.wh_plan_days(
            datetime.now(), -1))
    ...
    def _calc_qualified_demand(self, current_date=False):
        ...
        for rec in self:
            ...
            dates = list(
                set(demand_by_days.keys()) | set(mrp_moves_by_days.keys()))
            for date in dates:
    ...
    def cron_ddmrp_adu(self, automatic=False):
        ...
        for b in buffers:
            try:
                i += 1
                _logger.debug("ddmrp cron_adu: %s. (%s/%s)" % (b.name, i, j))
                if automatic:
                ...
                _logger.exception(
                    "Fail to compute ADU for buffer %s", b.name)
                if not automatic:
                    raise
    ...
    def cron_ddmrp(self, automatic=False):
        ...
        for b in buffers:
            i += 1
            _logger.debug("ddmrp cron: %s. (%s/%s)" % (b.name, i, j))
            try:
                if automatic:
                    with self.env.cr.savepoint():
                        b.cron_actions()

...
class StockWarehouseOrderpoint(models.Model):
    ...
    def _compute_ddmrp_chart(self):
        ...
        for rec in self:
            ...
            nflow = p.line([0, 2], [rec.net_flow_position,
                                      rec.net_flow_position], line_width=2)
            onhand = p.line([0, 2], [rec.product_location_qty,
            ...
            legend = Legend(items=[
                ...
                ("Yellow zone", [yellow]),
                ("Green zone", [green]),
                ("Net Flow Position", [nflow]),
                ("On-Hand Position", [onhand]),
            ])
            source = ColumnDataSource(
                data=dict(height=[rec.net_flow_position,
                                  rec.product_location_qty,
                                  rec.top_of_red,
                                  rec.top_of_yellow,
                                  rec.top_of_green],

    ...
    def test_adjustment_generation(self):
        ...
        values = {
            'DAF': {
                ...
                getattr(self, 'month_%i_%i' % (
                    next_month.year, next_month.month)): 2,
                getattr(self, 'month_%i_%i' % (
                    following_month.year, following_month.month)): 1.8
            },
            'LTAF': {
                getattr(self, 'month_%i_%i' % (
                    self.now.year, self.now.month)): 2,
                getattr(self, 'month_%i_%i' % (
                    next_month.year, next_month.month)): 2.5,
                getattr(self, 'month_%i_%i' % (
                    following_month.year, following_month.month)): 2
            }
        }
